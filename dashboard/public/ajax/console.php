...
<?php elseif (get("action") == 'update' && get("id")): ?>
  <?php
    $consoleHistory = $db->prepare("SELECT CH.*, A.realname FROM ConsoleHistory CH INNER JOIN Accounts A ON CH.accountID = A.id WHERE CH.serverID = ? ORDER BY CH.id ASC LIMIT 100");
