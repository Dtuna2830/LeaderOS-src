  ...
  if ($dashboardChart.length) {
    ...
    $.ajax({
      ...
      success: function(result) {
        ...
        $earnChartValue.find("h3").html(result.yearEarnedMoneyValue + '<small> <i class="fa fa-coins"></i></small>').css("display", "block");
      }
    }).then(function() {
  ...
  if ($thisMonthEarnData.length) {
    $.ajax({
      ...
      success: function(result) {
        ...
        $thisMonthEarnData.find(".h2").html(result.thisMonthEarnedMoney + '<small> <i class="fa fa-coins"></i></small>').css("display", "inline-block");
