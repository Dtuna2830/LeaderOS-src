...
function checkInstallStatus() {
  $.ajax({
    ...
    success: function(result) {
      if (result == true) {
        $("#redirect").attr("href", "/yonetim-paneli").html("Yönetim Paneline Git");
      }
      else {
        checkInstallStatus();
      }
    ...
    error: function(result) {
      console.log(result);
      $(".alert.alert-danger").css("display", "block").html('Bilinmeyen bir hata oluştu. Lütfen <strong>LeaderOS</strong> ile iletişime geçiniz! (ERROR_INSTALL_CHECK)');
    }
  });
}
  ...
  $(".btn-next, button[type=submit]").on("click", function() {
    ...
    if (nextStep == true) {
      ...
      $.ajax({
        ...
        error: function() {
          $(".alert.alert-danger").css("display", "block").html('Bilinmeyen bir hata oluştu. Lütfen <strong>LeaderOS</strong> ile iletişime geç!');
          loader.css("height", "auto").removeClass("is-loading");
          spinner.css("display", "none");
        }
      });
