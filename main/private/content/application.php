      ...
      <div class="col-md-12">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Yetkili Başvuru</li>
          </ol>
        </nav>
      </div>
      <div class="col-md-12">
        <?php if (get("action") == 'apply' && isset($_GET["form"])): ?>
          ...
          <?php if ($form->rowCount() > 0): ?>
            ...
            <?php if (($activeApplication->rowCount() == 0) || ($activeApplication->rowCount() > 0 && $readActiveApplication["status"] == "0" && $readForm["reappliable"] == "1")): ?>
              ...
              if (isset($_POST["apply"])) {
                if (!$csrf->validate('apply')) {
                  ...
                  echo alertSuccess("Başvurunuz başarıyla gönderildi! Yönlendiriliyorsunuz...");
                  echo goDelay("/yetkili-basvuru/$applicationID", 2);
                }
              }
              ?>
              <div class="card">
                  ...
                  <form action="" method="post">
                    ...
                    <div class="clearfix">
                      <div class="float-right">
                        <button type="submit" class="btn btn-success btn-rounded" name="apply">Gönder</button>
                      </div>
                    </div>
                  </form>
                </div>
            ...
            <?php else: ?>
              <?php echo alertError("Aktif bir başvurunuz varken tekrar başvuru yapamazsınız!") ?>
            <?php endif; ?>
          <?php else: ?>
            <?php echo alertError("Başvuru formu bulunamadı!") ?>
          <?php endif; ?>
        <?php elseif (get("action") == 'get' && isset($_GET["id"])): ?>
                ...
                <div class="row">
                  ...
                  <div class="col-auto">
                    <?php if ($readApplication["status"] == 0): ?>
                      <span class="badge badge-pill badge-danger" data-toggle="tooltip" data-placement="top" data-original-title="Durum">Reddedildi</span>
                    ...
                    <?php elseif ($readApplication["status"] == 2): ?>
                      <span class="badge badge-pill badge-warning" data-toggle="tooltip" data-placement="top" data-original-title="Durum">Onay Bekleniyor</span>
                    <?php else: ?>
          ...
          <?php else: ?>
            <?php echo alertError("Başvuru bulunamadı!"); ?>
          <?php endif; ?>
        <?php else: ?>
          <?php go('/404'); ?>
        <?php endif; ?>
