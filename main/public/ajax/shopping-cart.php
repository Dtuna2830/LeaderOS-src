  ...
  if (isset($_SESSION["login"])) {
    ...
    foreach ($shoppingCartProducts as $shoppingCartProduct) {
      ...
      $shoppingCartProductsArray[] = [
        ...
        "discountedPrice" => (int)ceil($discountedPrice),
        ...
        if ($readAccount["credit"] >= $subtotal) {
          ...
          if ($readSettings["webhookStoreURL"] != '0') {
            foreach ($shoppingCartProductsArray as $shoppingCartProduct) {
              ...
              $webhookProductName = $shoppingCartProduct["name"];
              if ($shoppingCartProduct["quantity"] > 1) $webhookProductName .= " x".$shoppingCartProduct["quantity"];
              $search = array("%username%", "%server%", "%product%");
              $replace = array($readAccount["realname"], $shoppingCartProduct["server"], $webhookProductName);
          ...
          if ($readSettings["oneSignalAppID"] != '0' && $readSettings["oneSignalAPIKey"] != '0') {
            ...
            if ($notificationPermission->rowCount() > 0) {
              ...
              if ($adminAccounts->rowCount() > 0) {
                ...
                $oneSignalProductList = [];
                foreach ($shoppingCartProductsArray as $shoppingCartProduct) {
                  $oneSignalProductList[] = $shoppingCartProduct["name"] . " x" . $shoppingCartProduct["quantity"];
                }
                $oneSignal = new OneSignal($readSettings["oneSignalAppID"], $readSettings["oneSignalAPIKey"], $oneSignalIDList);
                $oneSignal->sendMessage('LeaderOS Bildirim', $readAccount["realname"].' adlı kullanıcı '.implode(", ", $oneSignalIDList).' satın aldı.', '/yonetim-paneli/magaza/magaza-gecmisi');
              }
            }
          }
